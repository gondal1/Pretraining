...
class Vgg10:
    """
    A trainable version VGG10.
    """

    def __init__(self, vgg10_npy_path=None, trainable=True):
        if vgg10_npy_path is not None:
            self.data_dict = np.load(vgg10_npy_path, encoding='latin1').item()
        else:
            self.data_dict = None
    ...
    def get_fc_var(self, in_size, out_size, name):
        # initial_value = tf.truncated_normal(, 0.0, 0.1)
